...
?>
							   ...
							   $_GET['hadir'] = isset($_GET['hadir']) ? $_GET['hadir'] : '';
							   	   $_GET['bln'] = isset($_GET['bln']) ? $_GET['bln'] : '';
			   

	if ($_GET['hadir']<>'100' and $_GET['bln'] <>'100')
	{


$query = "SELECT * FROM berkas where hadir='$_GET[hadir]' and  YEAR(tanggal) = '$_GET[thn]' and Month(tanggal) = '$_GET[bln]' ORDER BY  id asc";
	}
	else if ($_GET['hadir']=='100' and $_GET['bln'] <>'')
	{
$query = "SELECT * FROM berkas where  YEAR(tanggal) = '$_GET[thn]' and Month(tanggal) = '$_GET[bln]' ORDER BY  id asc";
	}
	else if ($_GET['hadir'] <>''and $_GET['bln']=='100' )
	{
$query = "SELECT * FROM berkas where hadir='$_GET[hadir]' and  YEAR(tanggal) = '$_GET[thn]'  ORDER BY  id asc";
	}
	
	
	 if ($_GET['hadir']=='100' and $_GET['bln'] =='100')
	{
$query = "SELECT * FROM berkas where  YEAR(tanggal) = '$_GET[thn]'  ORDER BY  id asc";
